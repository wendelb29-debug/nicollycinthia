    ...
    <div className="min-h-screen bg-gradient-soft">
      ...
      <main className="mx-auto max-w-6xl px-6 py-12">
            ...
            {data?.plan?.services && data.plan.services.length > 0 && (
              <div className="mt-6 flex flex-wrap gap-2">
                {data.plan.services.map((s) => (
                  <span key={s} className="rounded-full border border-border bg-card/80 px-3 py-1 text-xs text-foreground">
                    {s}
                  </span>
                ))}
              </div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-3">
          ...
          <div className="rounded-3xl border border-border bg-card p-8 shadow-soft">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              {data?.address ?? "Endereço ainda não cadastrado."}
            </p>
            {data?.neighborhood && (
              <p className="mt-1 text-xs text-muted-foreground">{data.neighborhood}</p>
            )}
            <Button
              variant="ghost"
              size="sm"
              className="mt-4 px-0 text-gold hover:text-gold"
              onClick={() => toast.info("Edição de dados em breve.")}
            >
              Editar dados →
            </Button>
          </div>
        ...
        <div className="mt-10 flex items-center gap-3 rounded-2xl border border-gold/30 bg-gradient-rose/40 p-5 text-sm text-foreground">
          ...
          <p>
            Você faz parte de um clube com apenas 20 vagas. Aproveite cada detalhe — você merece.
          </p>
        </div>
      </main>
    </div>
